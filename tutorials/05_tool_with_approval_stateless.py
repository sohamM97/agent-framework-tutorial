...
# Point load_dotenv() straight at the .env next to this file.
# Why: with no path, it guesses where to look — and when a debugger is
# attached (e.g. VS Code) it searches from the current working directory
# instead of this script's folder, so it misses tutorials/.env and the
# env vars come back empty. Giving the exact path avoids that guessing.
...
async def handle_approvals(query: str, agent: "SupportsAgentRun") -> AgentResponse:
    ...
    # SOHAM: First, we run the query from the LLM. The results will have zero
    # or more user input requests. We loop through them and append each
    # request, as well as the user's approval response (True/False in boolean),
    # to the original message. Finally, we pass these messages
    # (original + approval requests + user responses) to the LLM again and run
    # the query. We keep doing this in a loop until there are no more approval
    # requests left. Note - we do all this because we don't use sessions here:
    # since it is stateless, we append all messages.
        ...
        for user_input_needed in result.user_input_requests:
            ...
            new_inputs.append(
                Message(
                    "user",
                    [
                        user_input_needed.to_function_approval_response(
...
async def handle_approvals_streaming(query: str, agent: "SupportsAgentRun") -> None:
    ...
    # SOHAM: logic is similar to the non-streaming version as mentioned in
    # handle_approvals. Except that here, we check whether each chunk has user
    # requests. If they do, we add them to a list while processing chunks.
    # After all chunks are processed, we go through the list, append the
    # requests and their user responses with the original query as earlier.
    # Then, in the next pass of the loop, we pass these messages to the LLM
    # again. We keep doing this until there are no more approval requests.
